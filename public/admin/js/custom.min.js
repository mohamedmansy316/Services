...
$(document).ready(function(){
    $('#admin.search-bar').click(function(){
    ...
    $('#admin-search-bar').keyup(function(){
        ...
        let ActionRoute = $(this).data('target');
        ...
        $.ajax({
            ...
            success: function(Response){
                $('#admin-input-search-results-products').fadeIn('fast');
                $('#admin-input-search-results-products').removeClass("admin-input-search-results-error");
                $('#products-search').html('');
                $.each(Response , function(i,item){
                    let imageUrl = `https://peppersluxury.com/admin/products/edit/${item.id}`;
                    let brandUrl = `https://peppersluxury.com/admin/brands/edit/${item.id}`;
                    $('#products-search').append(`
                        <div class="single-product-admin-search">
                            <a href="${imageUrl}" title="${item.title}" class="product-image">
                                <img class="admin-product-small-image" src="${item.SmallThumb}" alt="${item.title}" />
                            </a>
                            <div class="moreinfo">
                                <h4><a href="${brandUrl}">${item.brand.title}</a></h4>
                                <h3><a href="${imageUrl}" title="${item.title}">${item.title}</a></h3>
            ...
            error: function(Response){
                $('#admin-input-search-results-products').fadeIn('fast').addClass("admin-input-search-results-error");
                $('#products-search').html('');
                $('#products-search').append(`
                    <p class="text-center">${Response.responseText}</p>
                <br>`);
            }
        });
    });
    ...
    $('.body-overlay').click(function(){
        $('#admin-input-search-results-products').fadeOut('fast');
        $(this).fadeOut('fast');
    });
});
